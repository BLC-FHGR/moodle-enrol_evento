...
function xmldb_enrol_evento_install() {
    ...
    $uifidcert = $DB->get_records('user_info_field', array('shortname' => ENROL_EVENTO_UIF_COVIDCERT));

    if (empty($uifidcert)) {
        // Inserts new user_info_data item.
        $item = new \stdClass();
        $item->shortname = ENROL_EVENTO_UIF_COVIDCERT;
        $item->name = "Covid Certificate";
        $item->datatype = 'text';
        $item->description = "<p>Covid Certificate info</p>";
        $item->descriptionformat = FORMAT_HTML;
        $item->categoryid = 1;
        $item->required = 0;
        $item->locked = 1;
